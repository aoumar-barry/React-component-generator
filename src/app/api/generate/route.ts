...
import { 
  generateComponentStream as generateOpenAIStream, 
  validateReactComponentRequest as validateOpenAI,
  generateHelpfulResponseStream as generateOpenAIHelpfulResponse
} from '@/lib/openai';
import { 
  generateComponentStream as generateGeminiStream, 
  validateReactComponentRequest as validateGemini,
  generateHelpfulResponseStream as generateGeminiHelpfulResponse
} from '@/lib/gemini';
  ...
  try {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          // First, validate if the request is React component related
          ...
          if (!validation.isValid) {
            ...
            // If valid, proceed with code generation
            generator = provider === 'openai' 
          ...
          const errorData = JSON.stringify({ error: `Failed to generate component: ${errorMessage}` });
